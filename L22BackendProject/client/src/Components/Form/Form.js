...
const Form = () => {
  ...
  const createWorkout = async (e) => {
    ...
    try {
      ...
      console.error('Error creating workout:', err.message);
      setError(
        err.response?.data?.error || 'Failed to create workout. Please try again.'
      );
    } finally {
      setLoading(false);
    }
    ...
    try {
      const response = await axios.patch(
        `http://localhost:4000/api/workouts/${updateForm._id}`,
        {
          ...
          reps: updateForm.reps,
          load: updateForm.load,
      ...
      // Update state with edited workout
      setWorkouts(
        workouts.map((workout) =>
          workout._id === updateForm._id ? response.data : workout
        )
      );
      setUpdateForm({}); // Clear update form
      setSuccess(true);
    } catch (err) {
      console.error('Error updating workout:', err.message);
      setError(
        err.response?.data?.error || 'Failed to update workout. Please try again.'
      );
    } finally {
      setLoading(false);
    }
